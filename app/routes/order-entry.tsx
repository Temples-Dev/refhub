...
export default function OrderEntry() {
  ...
  const calculateTotal = () => {
    return items.reduce((total, item) => total + item.price * item.quantity, 0) + TRANSPORTATION_FEE;
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f5f6fa] to-[#e0e6ed] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
        ...
        <h2 className='font-bold py-4 text-2xl'>Welcome, Shelemiah</h2>
          <h1 className="text-3xl font-bold text-[#2c3e50] mb-6">New Order</h1>
          <Form className="space-y-6">
            <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              <div className="sm:col-span-3">
                <label htmlFor="itemName" className="block text-sm font-medium text-[#2c3e50]">
                  Item Name
                </label>
                <input
                  type="text"
              ...
              <div className="sm:col-span-2">
                <label htmlFor="price" className="block text-sm font-medium text-[#2c3e50]">
                  Price
                </label>
                <input
                  type="number"
              ...
              <div className="sm:col-span-1">
                <label htmlFor="quantity" className="block text-sm font-medium text-[#2c3e50]">
                  Quantity
                </label>
                <input
                  type="number"
          ...
          {showPreview && items.length > 0 && (
            <div className="mt-8 bg-gray-50 p-4 rounded-md">
              <h2 className="text-lg font-medium text-[#2c3e50] mb-4">Order Preview</h2>
              <ul className="divide-y divide-gray-200">
                {items.map((item) => (
                  <li key={item.id} className="py-4 flex justify-between items-center">
                    <div>
                      <p className="text-sm font-medium text-[#2c3e50]">{item.name}</p>
              ...
              <div className="mt-4 flex justify-between items-center">
                ...
                <p className="text-sm font-medium text-[#2c3e50]">${TRANSPORTATION_FEE.toFixed(2)}</p>
              </div>
              <div className="mt-4 flex justify-between items-center">
                <p className="text-lg font-medium text-[#2c3e50]">Total Amount:</p>
                <p className="text-lg font-bold text-[#2c3e50]">${calculateTotal().toFixed(2)}</p>
              </div>
              <div className="mt-6">
                <button
                  type="submit"
